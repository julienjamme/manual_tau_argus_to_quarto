...
The Main window
Starting with the Open Source version (4.0) the main window of τ-argushas been changed completely. In the previous versions an overview waspresented of the number of unsafe combinations for each explanatoryvariable and each code. However this information was hardy used andthe focus of the user is on the table(s) itself. So from now on thetable itself is the central point (the main window) of τ-argus. Assoon as the table has been completed, the table is presented here andthe process of disclosure control is controlled from this main window.
